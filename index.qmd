...
SSPCloud allows users to launch pre-configured workspaces through a simple HTTPS link. Doing so, there is no need for any local installation or complex setup: learners can start working immediately, directly in their browser. You can find some [tutorials](https://www.sspcloud.fr/catalog) examples.
...
While `learnr` allows for the creation of interactive and advanced elements such as quizzes—making it well-suited for beginner tutorials based a Rmarkdown file—it has some important limitations.
...
If you're using SSPCLoud for the first time, you must configure access to your GitHub repository you want to work on (see [version control](https://docs.sspcloud.fr/content/version-control.html) for more details).
You don't have to configure your GitHub access every time afterwards.
:::
...
:::: {.callout-warning collapse="false"}
__You should use `pip install` _anywhere_ in your notebooks__ because it is not the proper way to manage package requirements (see below for recommendations on dependency management).
::::
...
The script `init-trainees.sh` provided in the `sspcloud` folder of the _template_ helps you to configure the environment for trainees on the SSPCloud. This script does two things: install the environment needed to run the notebook (this is greatly facilitated by the use of `uv`) and download the notebook you want to provide trainees with. 
...
The script `init-trainees.sh` provided in the `sspcloud` folder of the _template_ helps you to configure the environment for trainees on the SSPCloud. This script does two things: install the environment needed to run the notebook (using `renv`) and open the Quarto file you want to provide trainees with. 
