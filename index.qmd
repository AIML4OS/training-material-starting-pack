...
This tutorial provides a practical guide for developing educational resources in {{< fa brands r-project >}} or `Python` {{< fa brands python >}} and sharing them easily.

To achieve this, certain technical prerequisites are required, detailed in @imp-req.

This tutorial aims to quickly equip you with the skills to build educational resources with __state-of-the-art level of reproducibility__. The next sections explain how to develop educational resources, and how to disseminate them on the SPPCloud. Keep in mind that this tutorial is intended for people developing resources, not for the users of these resources.


::: {#imp-req .callout-important collapse="false"}
## Technical requirements to deploy educational resources

- A minimum level of proficiency in `Git` {{< fa brands git-alt >}} is required to develop and make available online training resources. However, accessing these resources on the SSPCloud does not require any familiarity with Git.
...
# Benefits of sharing educational resources on the SSPCloud

Sharing educational resources on the SSPCloud offers several advantages, particularly for educators and learners working with computational tools like R or Python. The key benefits include :

1. **Instant access via a simple link**
SSPCloud allows users to launch pre-configured workspaces through a simple HTTPS link. This eliminates the need for local installation or complex setup: learners can start working immediately, directly in their browser.
...
__Be sure to regularly commit and push your changes__ (every 30 minutes or so), because your service (Rstudio or VScode) is not persistent, meaning that any changes that were pushed before closing the service are permanently lost.
:::
...
You can create lists and nested sublists :
...
Here's how to use `renv`:

- The first step in using `renv` is to initiate the tracking of dependencies using `renv::init()`. __This step is unnecessary if you’re using the `R` template__ because this was already done in the template.
- When developing resources, you should regularly run `renv::snapshot()`. This command analyzes your `R` and Quarto scripts, detects what packages are used, and updates the `renv.lock` file. Do not forget to commit and push the changes to the `renv.lock` file!
...
::: {.callout-warning collapse="false"}
If you forget to update the `renv.lock` file, your Github Actions workflow is likely to fail, and you probably won't be able to execute your `R` script in a new RStudio service. Don't worry, here's the solution: just run `renv::snapshot()`, commit and push the changes to the `renv.lock` file. This should solve the problem.
:::
...
The script `init-trainees.sh` provided in the `sspcloud` folder of the _template_ helps you to configure the environment for trainees on the SSPCloud. This script does two things: install the environment needed to run the notebook and download the notebook you want to provide trainees with. 
...
The script `init-trainees.sh` provided in the `sspcloud` folder of the _template_ helps you to configure the environment for trainees on the SSPCloud. This script does two things: install the environment needed to run the notebook and open the Quarto file you want to provide trainees with. 
...
When using initialization script, you **must put** the __raw github URL__ of the script in the `Initialization scripts` field of the SSPCloud configuration of your service. This URL is available by clicking on the `raw` button when opening the file on Github (see screenshot below). The URL should have a pattern similar to this one: <https://raw.githubusercontent.com/InseeFrLab/AIML4OS-template-quarto-python/refs/heads/main/sspcloud/init-trainees.sh>.
